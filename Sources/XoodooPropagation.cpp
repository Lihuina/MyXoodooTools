...
void XoodooPropagation::initializeAffine()
{
		...
		for (ColumnValue column = 0; column < (1 <<nrColumns); column++) {
			AffineSpaceOfColumns a;
			if (column == ((1 << nrColumns) - 1)) {
				a.setOffset(0x7);
				a.addGenerator(0x3);
				a.addGenerator(0x6);
			}
...
{
    chiCompatibilityTable.assign((1<<nrColumns)*(1<<nrColumns), false);
    for(ColumnValue a=0; a<(1<<nrColumns); a++) {
        vector<ColumnValue> list;
        affinePerInput[a].getAllColumnValues(list);
        for(unsigned int i=0; i<list.size(); i++)
            chiCompatibilityTable[a+(1<<nrColumns)*list[i]] = true;
    }
}
